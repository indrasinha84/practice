...
public class SubstringWithConcatenationofAllWords {
	...
	public static void main(String[] args) {
		...
		s.findSubstring("foobarhellofoobarhi", new String[] { "foo", "bar", "hi" });
	}

	public List<Integer> findSubstring(String s, String[] words) {
		List<Integer> positions = new ArrayList<Integer>();
		int allowedPosition = -1;
		for (String word:words) {
			int currentIndex = s.indexOf(word);
			int wordLength = word.length();
			if (allowedPosition == -1 || currentIndex == allowedPosition) {
				allowedPosition = currentIndex + wordLength;
				positions.add(currentIndex);
			}
			else if (allowedPosition != -1 && currentIndex != allowedPosition) 
